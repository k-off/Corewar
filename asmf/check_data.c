...
static void	check_size(t_operation *op)
{
	...
	if (size != op->size)
		exit(ft_printf("ERROR: wrong operation size, op. %d\n", op->unique_id));
}
...
static void	check_labels(t_operation *oper, t_label *label, int arg_nr)
{
	...
	if (tmp->position == -1)
		exit(ft_printf("ERROR: label %d is not set\n", tmp->unique_id));
	...
	if (oper->argument_type[arg_nr] == 1 &&
		...
		exit(ft_printf("ERROR: operation %d argument %d is registry %d - invld",
...
static void	check_operation(t_data *data, t_operation *oper)
{
	...
	if (oper->next && oper->unique_id - oper->next->unique_id != -1)
		exit(ft_printf("ERROR: operation nr %d enumeration sequence is wrong\n",
			oper->unique_id));
...
void		check(t_data *data)
{
	...
	if (ft_strlen(data->name) > PROG_NAME_LENGTH)
		exit(ft_printf("ERROR: Champ name length (%d) > maximum length (%d)\n",
			ft_strlen(data->name), PROG_NAME_LENGTH));
	if (ft_strlen(data->comment) > COMMENT_LENGTH)
		exit(ft_printf("ERROR: Champ comment len. (%d) > maximum length (%d)\n",
