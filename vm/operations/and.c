...
/*                                                   +#+                      */
/*   Created: 2019/08/22 11:23:45 by lskrauci      #+#    #+#                 */
/*   Updated: 2019/08/22 11:23:46 by lskrauci      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
/*
** @desc If 'and' command is executed by carriage,
...
** @desc which actually executes the command. Thus carriage_and_hub
...
void	carriage_and_hub(t_carriage *carriage, unsigned char *arena)
{
	int		arg1;
	int		arg2;
	int		arg3;
	...
	if (types->type3 == 1 && types->type1 != 0 && types->type2 != 0)
	{
		...
		carriage_and_hub_assist_1(carriage, types, &arg1, &temp) == 0)
		{
			carriage_jump(carriage);
			free(types);
			return ;
		}
		if (carriage_and_hub2(carriage, types, &arg2, &temp) == 0)
			return ;
		carriage_and(carriage, arg1, arg2, arg3);
	}
	free(types);
	carriage_jump(carriage);
}
		...
		t_types *types, int *arg2, int *temp)
{
	if (types->type2 == 1)
	{
		if (carriage_and_hub_assist_2(carriage, types, arg2) == 0)
		{
			carriage_jump(carriage);
			free(types);
			return (0);
		...
		carriage_and_hub_assist_3(carriage, types, arg2);
	else
		carriage_and_hub_assist_4(carriage, types, arg2, temp);
	return (1);
}
