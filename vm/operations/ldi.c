...
/*                                                   +#+                      */
/*   Created: 2019/08/22 11:27:32 by lskrauci      #+#    #+#                 */
/*   Updated: 2019/08/22 11:27:33 by lskrauci      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
** @desc then carriage_ldi_hub extracts information
...
** @desc If ldi has received valid 3 arguments, it extracts
** @desc argument values and sends them to carriage_ldi function,
** @desc which actually executes the command. Thus carriage_ldi_hub
...
void	carriage_ldi_hub(t_carriage *carriage, unsigned char *arena)
{
	int		arg1;
	int		arg2;
	int		arg3;
	...
	&& types->type3 == 1 && types->type1 != 0)
	{
		...
		carriage_ldi_hub_assist_1(carriage, types, &arg1, &temp) == 0)
		{
			carriage_jump_free(carriage, types);
			return ;
		}
		if (carriage_ldi_hub2(carriage, types, &arg2) == 0)
			return ;
		carriage_ldi(carriage, arg1, arg2, arg3);
	}
	free(types);
	carriage_jump(carriage);
}

int		carriage_ldi_hub2(t_carriage *carriage, t_types *types, int *arg2)
{
	if (types->type2 == 1)
	{
		if (carriage_ldi_hub_assist_2(carriage, types, arg2) == 0)
		{
			carriage_jump(carriage);
			free(types);
			return (0);
		...
		carriage_ldi_hub_assist_3(carriage, types, arg2);
	return (1);
}
