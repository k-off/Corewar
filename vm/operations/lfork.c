...
/*                                                   +#+                      */
/*   Created: 2019/08/22 11:28:02 by lskrauci      #+#    #+#                 */
/*   Updated: 2019/08/22 11:28:03 by lskrauci      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
/*
** @desc If 'lfork' command is executed by carriage,
...
void	carriage_lfork_hub(t_carriage *carriage, unsigned char *arena)
{
	carriage->jump =
	calculate_jump(carriage->current_opcode, arena, carriage->current_position);
	carriage_lfork(carriage, extract_2_bytes(carriage->current_position + 1));
...
** @desc direct_value. 'lfork' uses direct_value % IDX_MOD.
...
void	carriage_lfork(t_carriage *carriage, int distance)
{
	...
	new_carriage = carriage_origin(player, arena,
	(carriage->current_position + distance) % 4096);
	free(player);
